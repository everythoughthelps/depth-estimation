...
class FeatureResnext(nn.Module):
    ...
    def __init__(self, clz_model):
        ...
        self.conv1 = clz_model[0]
        self.bn1 = clz_model[1]
        self.relu = clz_model[2]
        self.maxpool = clz_model[3]

        self.block1 = clz_model[4]
        self.block2 = clz_model[5]
        self.block3 = clz_model[6]
        self.block4 = clz_model[7]
    ...
    def forward(self, x):
        ...
        glo_fea = F.interpolate(self.global_pool(self.conv_reduce(x_block4)), size=x_block4.size()[2:], mode='nearest')

        return x_block1, x_block2, x_block3, x_block4, glo_fea


class FTB(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(FTB, self).__init__()
        self.conv_unify = nn.Conv2d(in_ch, out_ch, kernel_size=1, stride=1, padding=0)
...
class FeatureFusionModule(nn.Module):
    def __init__(self, in_ch_x, in_ch_y, out_ch):
        super(FeatureFusionModule, self).__init__()
        self.FTB1 = FTB(in_ch_x, in_ch_y)
...
class DABC(nn.Module):
    def __init__(self, fea_ext_module, num_classes, dropout=False):
        super(DABC, self).__init__()
        self.fea_ext_module = fea_ext_module

        self.F4 = FeatureFusionModule(2048, 512, 256)
        self.F3 = FeatureFusionModule(1024, 256, 256)
        self.F2 = FeatureFusionModule(512, 256, 256)
        self.F1 = FeatureFusionModule(256, 256, 256)

        self.prediction = nn.Sequential()
        if dropout:
            self.prediction.add_module('dropout', nn.Dropout(0.5))
